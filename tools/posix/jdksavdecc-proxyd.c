...
us_getopt_option_t proxyd_global_options[]
    = {{"help", "Get list of options", US_GETOPT_FLAG, &proxyd_option_help_default, &proxyd_option_help},
       {"dump", "Dump all current options", US_GETOPT_FLAG, &proxyd_option_dump_default, &proxyd_option_dump},
       {"detail",                "Log detail level (0=none, 1=errors, 2=warnings, 3=info, 4=debug, 5=trace)",
        US_GETOPT_UINT16,        &proxyd_option_log_level_default,
        &proxyd_option_log_level},
       {"syslog", "Log via syslog", US_GETOPT_FLAG, &proxyd_option_syslog_default, &proxyd_option_syslog},
       {"memory",                             "Maximum memory usage in KBytes", US_GETOPT_UINT32,
        &proxyd_option_memory_length_default, &proxyd_option_memory_length},
       {"identity", "Identity", US_GETOPT_STRING, &proxyd_option_identity_default, &proxyd_option_identity},
       {"home", "Home directory", US_GETOPT_STRING, &proxyd_option_homedir_default, &proxyd_option_homedir},
       {"pidfile", "pid file name", US_GETOPT_STRING, &proxyd_option_pidfile_default, &proxyd_option_pidfile},
       {"newuid", "New user ID", US_GETOPT_STRING, &proxyd_option_newuid_default, &proxyd_option_newuid}, {0, 0, 0, 0, 0}};

/**! The options beggining with proxyd */
us_getopt_option_t proxyd_options[]
    = {{"address", "IP address to listen on", US_GETOPT_STRING, &proxyd_option_address_default, &proxyd_option_address},
       {"port", "IP port to listen on", US_GETOPT_STRING, &proxyd_option_port_default, &proxyd_option_port},
       {"avbport",                    "The AVB ethernet interface to use for AVDECC messages", US_GETOPT_STRING,
        &proxyd_option_avbif_default, &proxyd_option_avbif},
       {"maxclients",                      "Maximum number of simultaneous proxy clients", US_GETOPT_UINT16,
        &proxyd_option_maxclients_default, &proxyd_option_maxclients},
       {"session",                                    "Maximum session memory usage in KBytes", US_GETOPT_UINT32,
        &proxyd_option_session_memory_length_default, &proxyd_option_session_memory_length},
       {0, 0, 0, 0, 0}};
 ...
 */
bool avdeccproxy_http_server_handler_init( us_reactor_handler_t *self_, us_allocator_t *allocator, int fd, void *extra )
{
    bool r = false;
    ...
    if ( us_http_server_handler_init( &self->m_base.m_base.m_base, allocator, fd, 0, 8192, params->web_director ) )
    {
 ...
 *  that is used
 */
bool avdeccproxy_http_server_init( us_reactor_handler_t *self, us_allocator_t *allocator, int fd, void *extra )
{
...
bool proxyd_avtp_packet_received( us_reactor_handler_rawnet_t *self,
...
bool proxyd_run( void )
{
    ...
    r = us_reactor_init( &reactor, proxyd_global_allocator, proxyd_option_maxclients );
    if ( r )
    {
        ...
        if ( h )
        {
            ...
            if ( us_reactor_handler_rawnet_init( h,
    ...
    if ( r )
    {
        ...
        r = us_reactor_create_server( &reactor,
                                      proxyd_global_allocator,
                                      0,
                                      JDKSAVDECC_APPDU_TCP_PORT_STRING,
                                      SOCK_STREAM,
                                      &params,
    ...
    if ( r )
    {
        while ( !us_platform_sigterm_seen && reactor.poll( &reactor, 2000 ) )
        {
            fprintf( stdout, "tick\n" );
        }
...
int main( int argc, const char **argv )
{
    ...
    if ( us_platform_init_sockets() && proxyd_logger_init() && proxyd_options_init( JDKSAVDECC_PROXYD_CONFIG_FILE, argv ) )
    {
        ...
        if ( us_rawnet_multi_open( &proxyd_net,
                                   JDKSAVDECC_AVTP_ETHERTYPE,
                                   jdksavdecc_multicast_adp_acmp.value,
                                   jdksavdecc_multicast_identification.value ) )
        {
            ...
            us_rawnet_multi_join_multicast( &proxyd_net, jdksavdecc_jdks_multicast_log.value );
